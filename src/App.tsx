...
function MediaCard({ item }: { item: MediaEntry }) {
  ...
  if (item.category !== 'game' && !item.url) return <div className="card game-card"><span className="tool-icon">{item.category === 'music' ? '♫' : '◉'}</span><h3>{item.title}</h3><p className="muted">{item.description}</p><p className="small muted">Local media support is ready—choose a file in your browser to play it.</p><input type="file" accept={item.mediaType === 'audio' ? 'audio/*' : 'video/*'} onChange={e => { const file = e.target.files?.[0]; if (file) { const element = document.getElementById(`media-${item.id}`) as HTMLMediaElement | null; if (element) element.src = URL.createObjectURL(file); } }} />{item.mediaType === 'audio' ? <audio id={`media-${item.id}`} controls /> : <video id={`media-${item.id}`} controls />}</div>;
  return <div className="card game-card"><span className="tool-icon">🎮</span><h3>{item.title}</h3><p className="muted">{item.description}</p>{failed ? <><p className="muted">This game could not be launched in the current browser.</p><button className="ghost" onClick={() => setFailed(false)}>Retry</button></> : <iframe key={String(failed)} src={item.url} title={item.title} sandbox="allow-scripts allow-same-origin allow-forms" onError={() => setFailed(true)} />}</div>;
}
