...
function Layout() {
  ...
  const links = [['/', '⌂', 'Dashboard'], ['/calculator', '＋', 'Calculator'], ['/converter', '⇄', 'Unit converter'], ['/whiteboard', '✎', 'Whiteboard'], ['/study/notes', '▤', 'Notes'], ['/study/flashcards', '▣', 'Flashcards'], ['/study/dictionary', '⌕', 'Dictionary'], ['/study/formulas', '∑', 'Formulas'], ['/study/pomodoro', '◷', 'Pomodoro'], ['/study/scientific', '∫', 'Scientific calc'], ['/games', '▶', 'Games & media'], ['/browser', '◉', 'Browser'], ['/insights', '◇', 'Risk insights'], ['/settings', '⚙', 'Settings']];
  return <div className={`app ${panic ? 'panic' : ''}`}><aside><div className="brand"><span>Li</span> Lithium</div><nav>{links.map(([to, icon, label]) => <NavLink key={to} to={to} end={to === '/'}><b>{icon}</b>{label}</NavLink>)}</nav><button className="panic-button" onClick={() => setPanic(true)}>Panic mode <kbd>Esc</kbd></button></aside><main className="content" id="main-content" tabIndex={-1}><header><div><span className="eyebrow">WELCOME BACK</span><h2>{user.name}</h2></div><button className="ghost" onClick={() => { storage.remove('user'); navigate('/auth'); }}>Sign out</button></header>{panic && <div className="panic-banner" role="alert"><strong>Panic mode active.</strong> Media and distractions are paused. <button onClick={() => setPanic(false)}>Return to Lithium</button></div>}<Routes><Route path="/" element={<Dashboard />} /><Route path="/calculator" element={<Calculator />} /><Route path="/converter" element={<Converter />} /><Route path="/whiteboard" element={<Whiteboard />} /><Route path="/study/notes" element={<Notes />} /><Route path="/study/flashcards" element={<Flashcards />} /><Route path="/study/dictionary" element={<Dictionary />} /><Route path="/study/formulas" element={<Formulas />} /><Route path="/study/pomodoro" element={<Pomodoro />} /><Route path="/study/scientific" element={<ScientificCalculator />} /><Route path="/games" element={<Games />} /><Route path="/browser" element={<Browser />} /><Route path="/insights" element={<Insights />} /><Route path="/settings" element={<Settings />} /></Routes></main></div>;
}
...
function Browser() {
  const initial = storage.get<BrowserTab[]>('browser-tabs', [{ id: 'new', url: 'about:blank', title: 'New tab' }]);
  const [tabs, setTabs] = useState(initial); const [active, setActive] = useState(initial[0]?.id ?? ''); const [address, setAddress] = useState('about:blank'); const [status, setStatus] = useState('ready'); const [popupBlocked, setPopupBlocked] = useState(false); const frame = useRef<HTMLIFrameElement>(null);
  const current = tabs.find(tab => tab.id === active);
  const update = (next: BrowserTab[]) => { setTabs(next); storage.set('browser-tabs', next); };
  const handleNavigate = () => { let url = address.trim(); if (!url) return; if (!/^https?:\/\//i.test(url) && !url.startsWith('about:')) url = `https://${url}`; try { if (url !== 'about:blank' && new URL(url).protocol !== 'https:') throw new Error(); } catch { setStatus('error'); return; } update(tabs.map(tab => tab.id === active ? { ...tab, url, title: url === 'about:blank' ? 'New tab' : url } : tab)); setStatus('loading'); };
  const addTab = () => { const tab = { id: crypto.randomUUID(), url: 'about:blank', title: 'New tab' }; update([...tabs, tab]); setActive(tab.id); setAddress(tab.url); setStatus('ready'); };
  useEffect(() => { if (current) setAddress(current.url); }, [active, current]);
  return <Tool title="Browser" description="A minimal embedded browser. External sites may restrict embedding."><div className="browser-container"><div className="tab-bar">{tabs.map(tab => <button className={`tab ${tab.id === active ? 'active' : ''}`} key={tab.id} onClick={() => setActive(tab.id)}>{tab.title}<span onClick={event => { event.stopPropagation(); const next = tabs.filter(item => item.id !== tab.id); const fallback = { id: crypto.randomUUID(), url: 'about:blank', title: 'New tab' }; const remaining = next.length ? next : [fallback]; update(remaining); setActive(remaining[0].id); }}>×</span></button>)}<button className="ghost" onClick={addTab}>＋</button></div><form className="browser-toolbar" onSubmit={event => { event.preventDefault(); handleNavigate(); }}><input value={address} onChange={event => setAddress(event.target.value)} aria-label="Browser address" /><button>Go</button><button type="button" className="ghost" onClick={() => { const popup = window.open(current?.url || 'about:blank', '_blank', 'noopener,noreferrer'); setPopupBlocked(!popup); }}>Open</button></form>{popupBlocked && <p className="browser-warning">Popup blocked. Allow popups or use the embedded view.</p>}<div className="browser-frame">{status === 'error' && <div className="browser-error">Invalid or restricted URL. <button onClick={handleNavigate}>Retry</button></div>}<iframe ref={frame} src={current?.url} title={current?.title} sandbox="allow-scripts allow-same-origin allow-forms allow-popups" onLoad={() => setStatus('ready')} onError={() => setStatus('error')} /></div></div></Tool>;
}
type Outcome = { tool: string; result: 'success' | 'failure'; at: string };
function Insights() {
  const [enabled, setEnabled] = useState(storage.get('predictive-consent', false));
  const [outcomes, setOutcomes] = useState<Outcome[]>(storage.get('outcomes', []));
  const successRate = outcomes.length ? Math.round(outcomes.filter(item => item.result === 'success').length / outcomes.length * 100) : 0;
  const record = (result: Outcome['result']) => { const next = [...outcomes, { tool: 'manual check', result, at: new Date().toISOString() }]; setOutcomes(next); storage.set('outcomes', next); };
  const exportData = () => { const blob = new Blob([JSON.stringify(outcomes, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'lithium-outcomes.json'; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1000); };
  return <Tool title="Risk insights" description="Experimental, local-only learning about study outcomes. Nothing is sent anywhere."><div className="card insight-card"><p className="eyebrow">EXPERIMENTAL</p><label className="check"><input type="checkbox" checked={enabled} onChange={event => { setEnabled(event.target.checked); storage.set('predictive-consent', event.target.checked); }} /> Allow opt-in outcome learning</label>{enabled ? <><h2>{successRate}% positive outcomes</h2><p className="muted">Based on {outcomes.length} local check-ins. Use these signals as suggestions, not guarantees.</p><p><button onClick={() => record('success')}>Record success</button><button className="ghost" onClick={() => record('failure')}>Record failure</button></p><p><button className="ghost" onClick={exportData}>Export data</button><button className="ghost" onClick={() => { setOutcomes([]); storage.remove('outcomes'); }}>Delete history</button></p></> : <p className="muted">Enable consent to see local pre-launch risk signals and feedback controls.</p>}</div></Tool>;
}
...
export default function App() { return <ErrorBoundary><Routes><Route path="/auth" element={<Auth />} /><Route path="/privacy" element={<Privacy />} /><Route path="/*" element={<Protected><Layout /></Protected>} /></Routes></ErrorBoundary>; }
