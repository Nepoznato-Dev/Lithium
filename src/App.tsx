...
function Layout() {
  ...
  const links = [['/', '⌂', 'Dashboard'], ['/calculator', '＋', 'Calculator'], ['/converter', '⇄', 'Unit converter'], ['/whiteboard', '✎', 'Whiteboard'], ['/study/notes', '▤', 'Notes'], ['/study/flashcards', '▣', 'Flashcards'], ['/study/dictionary', '⌕', 'Dictionary'], ['/study/formulas', '∑', 'Formulas'], ['/study/pomodoro', '◷', 'Pomodoro'], ['/study/scientific', '∫', 'Scientific calc'], ['/games', '▶', 'Games & media'], ['/browser', '◉', 'Browser'], ['/settings', '⚙', 'Settings']];
  return <div className={`app ${panic ? 'panic' : ''}`}><aside><div className="brand"><span>Li</span> Lithium</div><nav>{links.map(([to, icon, label]) => <NavLink key={to} to={to} end={to === '/'}><b>{icon}</b>{label}</NavLink>)}</nav><button className="panic-button" onClick={() => setPanic(true)}>Panic mode <kbd>Esc</kbd></button></aside><main className="content"><header><div><span className="eyebrow">WELCOME BACK</span><h2>{user.name}</h2></div><button className="ghost" onClick={() => { storage.remove('user'); navigate('/auth'); }}>Sign out</button></header>{panic && <div className="panic-banner" role="alert"><strong>Panic mode active.</strong> Media and distractions are paused. <button onClick={() => setPanic(false)}>Return to Lithium</button></div>}<Routes><Route path="/" element={<Dashboard />} /><Route path="/calculator" element={<Calculator />} /><Route path="/converter" element={<Converter />} /><Route path="/whiteboard" element={<Whiteboard />} /><Route path="/study/notes" element={<Notes />} /><Route path="/study/flashcards" element={<Flashcards />} /><Route path="/study/dictionary" element={<Dictionary />} /><Route path="/study/formulas" element={<Formulas />} /><Route path="/study/pomodoro" element={<Pomodoro />} /><Route path="/study/scientific" element={<ScientificCalculator />} /><Route path="/games" element={<Games />} /><Route path="/browser" element={<Browser />} /><Route path="/settings" element={<Settings />} /></Routes></main></div>;
}
