...
function Browser() {
  ...
  const navigate = () => { let url = address.trim(); if (!url) return; if (!/^https?:\/\//i.test(url) && !url.startsWith('about:')) url = `https://${url}`; try { if (url !== 'about:blank' && new URL(url).protocol !== 'https:') throw new Error(); } catch { setStatus('error'); return; } update(tabs.map(tab => tab.id === active ? { ...tab, url, title: url === 'about:blank' ? 'New tab' : url } : tab)); setStatus('loading'); };
