...
function Layout() {
  ...
  useEffect(() => { const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') setPanic(true); const tag = document.activeElement?.tagName; if (e.key === '/' && tag !== 'INPUT' && tag !== 'TEXTAREA' && tag !== 'SELECT') { e.preventDefault(); (document.querySelector('#quick-search') as HTMLInputElement)?.focus(); } }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler); }, []);
...
function Converter() {
  ...
  const result = category === 'temperature' ? from === 'celsius' && safeTo === 'fahrenheit' ? Number(amount) * 9 / 5 + 32 : from === 'fahrenheit' && safeTo === 'celsius' ? (Number(amount) - 32) * 5 / 9 : Number(amount) : Number(amount) * units.distance[from as keyof typeof units.distance] / units.distance[safeTo as keyof typeof units.distance];
