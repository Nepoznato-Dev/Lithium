...
use lithium_abi::{get, json_escape, num_json, write_json, Parser, Value};
        ...
        let params: Vec<String> = spec
            ...
            .map(|p| {
                ...
                if p.kind == Kind::Num && (p.min != f64::MIN || p.max != f64::MAX) {
                    s.push_str(&format!(",\"min\":{},\"max\":{}", num_json(p.min), num_json(p.max)));
                }
                s.push('}');
                s
            })
...
pub fn audit_append(input: &[u8]) -> Option<String> {
    ...
    entry.push_str(&format!("\"t\":{}", num_json(now)));
