// ../../../src/lib/aiBlocks.js
var blockRegex = (lang) => new RegExp(`\`\`\`${lang}[ \\t]*\\n([\\s\\S]*?)\`\`\``, "g");
function extractApiCalls(text) {
  const calls = [];
  let match;
  const regex = blockRegex("api");
  while ((match = regex.exec(text || "")) !== null) {
    try {
      const parsed = JSON.parse(match[1]);
      const list = Array.isArray(parsed) ? parsed : [parsed];
      for (const item of list) {
        if (item && typeof item.api === "string") calls.push({ api: item.api, params: item.params || {} });
      }
    } catch {
    }
  }
...
function extractWidgetBlocks(text) {
  const blocks = [];
  let match;
  const regex = blockRegex("widget");
  while ((match = regex.exec(text || "")) !== null) {
    const code = match[1].trim();
    const nameMatch = code.match(/\/\/\s*widget:\s*(.+?)\s*$/m);
    const name = (nameMatch ? nameMatch[1] : `AI Widget ${blocks.length + 1}`).replace(/[\\/:*?"<>|]/g, "").slice(0, 48);
    blocks.push({ name, code });
  }
  return blocks;
}
function stripToolBlocks(text) {
  return (text || "").replace(/```(?:api|widget)[ \t]*\n[\s\S]*?```/g, "").trim();
}
...
async function getCatalog() {
  await coreReady();
  const native = apiCatalogSync();
  if (native) return native;
  return [...HANDLERS.keys()].map((api) => ({
    api,
    ns: api.split(".")[0],
    desc: "",
    callers: ["system", "user", "widget", "model"],
    params: []
  }));
}
...
function engineInfo() {
  return { engine: hasWasm() ? "rust-wasm" : "js-fallback", handlers: HANDLERS.size };
}
