...
async function readEntryContent(entry) {
  if (entry.idb) return await getBlob(entry.id) || "";
  return entry.content || "";
}
async function removeEntryDeep(tree2, id) {
  const doomed = doomedList(tree2, id);
  await Promise.all(
    tree2.filter((entry) => doomed.has(entry.id) && entry.idb).map((entry) => deleteBlob(entry.id))
  );
...
on('boot', () => {
  api.notify('Welcome back \u{1F44B}', 'Lithium desktop is ready.');
});
`
  },
  {
