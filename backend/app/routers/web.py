...
async def proxy(url: str, request: Request):
    ...
    try:
        ...
        log.error('Proxy fetch failed for %s %s: %s: %s',
                  request.method, _safe_url_for_log(current),
                  type(err).__name__, err)
        raise HTTPException(502, f'Proxy fetch failed: {type(err).__name__}') from err

    if response.status_code >= 400:
        log.warning('Upstream %s returned %d for %s',
                    parsed_target.netloc, response.status_code, _safe_url_for_log(current))
