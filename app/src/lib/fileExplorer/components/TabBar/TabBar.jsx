...
export default function TabBar({ windowed, closeSelf, minimizeSelf, maximizeSelf, isMaximized }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-0.5 overflow-hidden border-b border-white/[0.08] bg-[#151518] px-2 pt-1">
      <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto">
            ...
            <div
              ...
              className={`group flex items-center gap-1.5 rounded-t px-3 py-1.5 text-xs cursor-pointer transition-colors ${
                tab.id === activeTabId.value
                  ...
                  : 'text-white/50 hover:bg-[#1e1f23] hover:text-white/70'
              }`}
              onClick={() => switchTab(tab.id)}
            >
              <PngIcon name="Folder" size={12} className="shrink-0" color="#f59e0b" />
        ...
        <button
          className="flex items-center justify-center rounded-md p-1 text-white/40 hover:bg-[#222328] hover:text-white"
          onClick={addTab}
          title="New tab"
        >
          <PngIcon name="Plus" size={13} />
        </button>
      </div>
